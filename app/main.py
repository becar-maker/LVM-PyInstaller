...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def open_video(self):
        ...
        self.iir = None; self.roi = None; self.label.clear_roi()
    ...
    def toggle_play(self, playing: bool):
        ...
        if playing:
            ...
            self.timer.start(int(1000 / max(fps, 1))); self.btn_play.setText("Pause")
        else:
            self.timer.stop(); self.btn_play.setText("Play")

    def on_roi_changed(self, rect: QtCore.QRect):
        if rect is None:
            self.roi=None; self.lbl_roi.setText("ROI: full frame"); self.iir=None
        else:
    ...
    def next_frame(self, draw_only=False):
        ...
        if not ok: self.btn_play.setChecked(False); return
        ...
        if self.iir is None:
            fps=float(self.spin_fps.value()); low=float(self.spin_low.value()); high=float(self.spin_high.value())
